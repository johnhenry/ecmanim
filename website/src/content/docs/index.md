...
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="/assets/ecmanim-logo-dark.png">
  <source media="(prefers-color-scheme: light)" srcset="/assets/ecmanim-logo-light.png">
  <img alt="ecmanim" src="/assets/ecmanim-logo-light.png" width="450">
</picture>
...
bundling them; see [docs/external-tools.md](/guides/external-tools/) for the
...
Or from the CLI (see [docs/cli.md](/guides/cli/)):
...
| `ecmanim/authoring` | Node | plan IR / formats | plan-IR dry-run, quality gates, pluggable Format lifecycle + llm/tts/render providers ([docs/authoring-studio.md](/guides/authoring-studio/)) |
| `ecmanim/studio` | Node + browser | live-preview dev server | hot-reloading `<manim-player>` preview + schema→props controls ([docs/authoring-studio.md](/guides/authoring-studio/)) |
...
→ billboards). Architecture details: [docs/architecture.md](/guides/architecture/).

Two **alternate render targets** share the same scene graph — see
[docs/renderers.md](/guides/renderers/):
...
[docs/cli.md](/guides/cli/).

## Plugins

Extend the engine three ways — see [docs/plugins.md](/guides/plugins/):
...
| `examples/video.ts` | VideoMobject — ingest an external clip (frames + audio), play it in-scene ([docs/video.md](/guides/video/)) |
...
| `examples/metadata.ts` | schema.org `VideoObject` + IIIF manifest export — chapters from `nextSection()` ([docs/metadata.md](/guides/metadata/)) |
| `examples/primitives.ts` | Timeline + `wiggle` driver + `VectorDecimalNumber` + style preset + `renderStill` ([docs/primitives.md](/guides/primitives/)) |
| `examples/audio-reactive.ts` | FFT spectrum bars + `CaptionTrack` + muxed audio ([docs/captions-audio.md](/guides/captions-audio/)) |
| `examples/voiceover.ts` | TTS-synced narration with `<bookmark>` cues ([docs/voiceover.md](/guides/voiceover/)) |
| `examples/diagram.ts` | diagram-as-code + animated board transition via auto-matching ([docs/animation-presentation.md](/guides/animation-presentation/)) |
| `examples/interchange.ts` | watermarked render + `.otio` + Lottie export ([docs/interchange.md](/guides/interchange/)) |
| `examples/physics.ts` | analytic E-field + pendulum + bouncing rigid bodies ([docs/physics.md](/guides/physics/)) |
| `examples/authoring.ts` | Format lifecycle → real render + plan-IR dry-run ([docs/authoring-studio.md](/guides/authoring-studio/)) |
...
[docs/architecture.md](/guides/architecture/).
