...
manim-js knows a video's structure at render time — duration, dimensions, and
...
- **provenance sliver** — "generated by manim-js" + the IPTC digital-source-type
...
isomorphic (no Node/DOM at import) and exported from `manim-js`.
...
```js
import { render, toVideoObject, toIIIFManifest } from "manim-js/node";
...
const input = {
  ...
  provenance: true,                     // adds the manim-js creator + IPTC term
};
```
...
import { toVideoObjectScript } from "manim-js";
...
```js
import { loadVideo } from "manim-js/node";       // or "manim-js/browser"
